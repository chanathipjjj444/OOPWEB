...
class Booking:
            ...
            for room in hotel.room_list:
                if room.room_number == room_number: #check เลขห้อง
                    if room.room_status == True:
                        ...
                        print("Room price:",room.price_room)
                        return True
                    else:
    ...
    def booking_add_on(self, hotel_name, addon, requirement, choices=["breakfast","spa","activity","taxi"]): #input hotel_name addon = service ที่ต้องการ choice คือ list service ทั้งหมด
        hotel = catalog.find_hotel(hotel_name)
        if hotel:
            hotel.show_add_on()
            for item in add_on_cat.add_on_list:
                if item[0] == addon and addon in choices:
                    if requirement == True:
                        print("Addon book success")
                        return item[2]
                    else:
                        print("Addon is not book")
                        return False
            else:
                print("This hotel has no addon")
        else:
            print("Unable to find hotel")    
...
breakfast2 = BreakfastService("breakfast2","beverage",50)
...
book1.booking_add_on("Hotel A","breakfast",True)